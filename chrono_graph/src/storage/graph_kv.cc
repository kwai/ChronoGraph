...
static base::MultiMemKV *GetKVByType(
    const std::unordered_map<std::string, std::unique_ptr<base::MultiMemKV>> &db, std::string relation_name);

#define SAFE_DB_BY_RELATION_RETURN_CODE()                                                           \
  auto db_kernel = GetKVByType(dbs_, relation_name);                                                \
  if (!db_kernel) {                                                                                 \
    LOG_EVERY_N(ERROR, 1000) << "Invalid Relation type, get null db, relation = " << relation_name; \
...
GraphStorageDBConfig GraphKV::ParseDBConfig(base::Json *config) {
  ...
  db_config.kv_shard_num = chrono_graph::GetInt64ConfigSafe(config, "kv_shard_num");
  db_config.kv_dict_capacity = chrono_graph::GetInt64ConfigSafe(config, "kv_dict_size");
  db_config.shard_memory_size = chrono_graph::GetInt64ConfigSafe(config, "kv_size");

  auto relation_name = chrono_graph::GetStringConfigSafe(config, "relation_name");
  ...
  if (!db_config.attr_op_config) {
    LOG(INFO) << relation_name << " attr_op_config not found, use default empty op.";
  ...
  if (!db_config.edge_attr_op_config) {
    LOG(INFO) << relation_name << " edge_attr_op_config not found, use default empty op.";
    db_config.edge_attr_op_config = &default_attr_op_config;
  }
  db_config.edge_max_num = chrono_graph::GetInt64ConfigSafe(config, "edge_max_num");
  db_config.expire_interval = chrono_graph::GetInt64ConfigSafe(config, "expire_interval");
  db_config.edge_expire_interval_s =
      config->GetInt("edge_expire_interval_s", db_config.edge_expire_interval_s);
  db_config.edge_expire_check_interval_s =
      config->GetInt("edge_expire_check_interval_s", db_config.edge_expire_check_interval_s);
  db_config.oversize_replace_strategy =
      config->GetNumber("oversize_replace_strategy", db_config.oversize_replace_strategy);
  db_config.max_occupancy_rate = config->GetNumber("max_occupancy_rate", db_config.max_occupancy_rate);
  db_config.reduce_occupancy_rate =
      config->GetNumber("reduce_occupancy_rate", db_config.reduce_occupancy_rate);
  db_config.reduce_occupancy_strategy =
      config->GetInt("reduce_occupancy_strategy", db_config.reduce_occupancy_strategy);
  db_config.elst = config->GetString("elst", "cpt");
  db_config.weight_decay_ratio = config->GetNumber("weight_decay_ratio", db_config.weight_decay_ratio);
  ...
  db_config.degree_decay_ratio = config->GetNumber("degree_decay_ratio", db_config.degree_decay_ratio);
  ...
  db_config.decay_interval_s = config->GetInt("decay_interval_s", db_config.decay_interval_s);
  db_config.delete_threshold_weight =
      config->GetNumber("delete_threshold_weight", db_config.delete_threshold_weight);
  return db_config;
}
...
void GraphKV::InitConfig() {
  ...
  auto service_name = chrono_graph::GetStringConfigSafe(json_config, "service_name");
  ...
  for (auto db_config : dbs_config) { config_.db_configs.push_back(ParseDBConfig(db_config)); }
}
...
bool GraphKV::Init(const std::string &flag_shm_dirs) {
  ...
  for (auto &db_config : config_.db_configs) {
    ...
    updaters_.emplace(relation_name, std::make_unique<GraphKvUpdater>(db_config));
    ...
    auto generate_slab_handler = [&](std::vector<uint32_t> *slabs) {
      ...
      CHECK(slabs->size() > 0) << "Unexpected slabs size: " << slabs->size() << ", check your config!";
      for (size_t i = 0; i < slabs->size(); ++i) {
        // each slab contains metadata from KVData, and the actual data.
        (*slabs)[i] += sizeof(base::KVData);
      }
      LOG(INFO) << "size of data in slabs = " << absl::StrJoin(*slabs, ",") << " for " << relation_name;
    };
    int expire_interval = (int)db_config.expire_interval;
    // Set expire time to 10 years by default.
    if (expire_interval <= 0) { expire_interval = 86400 * 365 * 10; }

    std::vector<std::string> final_shm_paths;
    for (auto shm_dir : shm_dirs) { final_shm_paths.push_back(shm_dir + "/" + relation_name); }

    CheckMetaInfo(GenerateMetaInfo(db_config), final_shm_paths);

    dbs_.emplace(
        relation_name,
        std::make_unique<base::MultiMemKV>(final_shm_paths,
  ...
  detection_thread_pool_ = std::make_unique<thread::ThreadPool>(dbs_.size() * 3);
  ...
  for (const auto &pair : dbs_) {
    ...
    detection_thread_pool_->AddTask([this, key]() { this->TimeDecayThread(key); });
    detection_thread_pool_->AddTask([this, key]() { this->EdgeExpireThread(key); });
  }
  LOG(INFO) << "Init DB Relation Set: [" << absl::StrJoin(available_relations, ", ") << "]";
  ...
  if (config_.checkpoint_config != nullptr) {
    ...
    if (!checkpoint_manager_->Init(all_db_empty())) { LOG(INFO) << "checkpoint init fail"; }
  }

  if (config_.init_config) {
    CHECK(config_.init_config->IsArray())
        << "init config is not an array, init fail, check your config, config = "
        << config_.init_config->ToString() << ", type is " << config_.init_config->Type();
    ...
    for (size_t i = 0; i < config_.init_config->size(); ++i) {
      ...
      CHECK(gnn_initers_[i].get()) << "Initer init fail, check your type name! i = " << i
  ...
  if (config_.sample_pool_config != nullptr) {
    CHECK(config_.sample_pool_config->IsArray())
        << "sample_pool config is not an array, init fail, check your config, config = "
        << config_.sample_pool_config->ToString() << ", type is " << config_.sample_pool_config->Type();
    ...
    for (size_t i = 0; i < pool_size; ++i) {
      ...
      CHECK(!name.empty()) << "Invalid config: sample pool name must be specified.";
      custom_rand_samplers_.emplace(name, new CustomSampleCache(this, pool_conf));
      CHECK(custom_rand_samplers_.at(name)->Init()) << "Custom rand sampler init fail";
    }
  }
...
bool GraphKV::ValidateBatchInsertRequest(const ::chrono_graph::BatchInsertRequest *request) {
  ...
  if (!request->attrs().empty() && (request->attrs().size() != insert_size)) {
    LOG_EVERY_N_SEC(WARNING, 5) << "batch insert error, src id size = " << insert_size
                                << ", attr size = " << request->attrs().size();
    return false;
  }
  if (!request->dest_ids().empty() && (request->dest_ids_size() != insert_size)) {
    LOG_EVERY_N_SEC(WARNING, 5) << "batch insert error, src id size = " << insert_size
                                << ", dst_ids size = " << request->dest_ids_size();
    return false;
  }
  if (!request->dest_weights().empty() && (request->dest_weights_size() != request->dest_ids_size())) {
    LOG_EVERY_N_SEC(WARNING, 5) << "batch insert error, dest ids size = " << dest_size
                                << ", dest weights size = " << request->dest_weights_size();
    return false;
  }
  if (!request->dst_ts().empty() && (request->dst_ts_size() != request->dest_ids_size())) {
    LOG_EVERY_N_SEC(WARNING, 5) << "batch insert error, dst ids size = " << dest_size
                                << ", dst ts size = " << request->dst_ts_size();
    return false;
  }
  if (!request->edge_attrs().empty() && (request->edge_attrs_size() != request->dest_ids_size())) {
    LOG_EVERY_N_SEC(WARNING, 5) << "batch insert error, dst ids size = " << dest_size
  ...
  if (!request->use_item_ts_for_expires().empty() &&
    ...
    LOG_EVERY_N_SEC(WARNING, 5) << "batch insert error, src ids size = " << insert_size
...
ErrorCode GraphKV::BatchInsert(const ::chrono_graph::BatchInsertRequest *request) {
  ...
  for (auto i = 0; i < request->src_ids().size(); ++i) {
    ...
    if (!request->dest_ids().empty()) { tmp_dests.push_back(request->dest_ids(i)); }
    if (!request->dest_weights().empty()) { tmp_w.push_back(request->dest_weights(i)); }
    if (!request->dst_ts().empty()) { tmp_ts.push_back(request->dst_ts(i)); }
    if (!request->edge_attrs().empty()) { tmp_edge_attrs.push_back(request->edge_attrs(i)); }
    if (!request->expire_ts_update_flags().empty()) {
    ...
    if ((i < request->src_ids().size() - 1) && (request->src_ids(i) == request->src_ids(i + 1))) { continue; }
    ...
    std::string src_attr = request->attrs().empty() ? "" : request->attrs().Get(i);
    timer.AppendCostMs("prepare params");
    auto code = InsertRelations(in_id,
                                tmp_dests,
                                request->relation_name(),
                                std::move(src_attr),
                                std::move(tmp_w),
                                std::move(tmp_ts),
                                std::move(tmp_edge_attrs),
                                std::move(tmp_expire_ts_update_flags),
                                request->iewa(),
                                false,  // overwrite
    ...
    if (timer.Stop() > 10000) {
      LOG_EVERY_N_SEC(WARNING, 3) << "insert key: " << in_id << " long time cost: " << timer.display()
                                  << ",  out_id size = " << tmp_dests.size();
    } else {
      LOG_EVERY_N_SEC(INFO, 5) << "insert key: " << in_id << " time cost: " << timer.display()
                               << ", out_id size = " << tmp_dests.size();
    }
  ...
  if (insert_time > 0) {
    ...
    PERF_AVG((end - begin) / insert_time, request->relation_name(), P_RQ_T, "insert_once");
  }
  return ErrorCode::OK;
}

ErrorCode GraphKV::BatchDelete(const ::chrono_graph::BatchInsertRequest *request) {
  auto begin = base::GetTimestamp();
  CHECK(request->is_delete());
  ...
  for (auto i = 0; i < request->src_ids().size(); ++i) {
    ...
    auto code = DeleteRelations(request->src_ids(i), tmp_dests, request->relation_name());
    if (code != ErrorCode::OK) { return code; }
    delete_time++;
    tmp_dests.clear();
  }
  ...
  if (delete_size > 0) {
    ...
    PERF_AVG((end - begin) / delete_time, request->relation_name(), P_RQ_T, "delete_once");
  }
  return ErrorCode::OK;
}

ErrorCode GraphKV::InsertRelations(uint64 in_id,
  ...
  if (!items.Set(db_config,
                 ...
                 std::move(out_ts),
                 std::move(edge_attrs),
                 std::move(expire_ts_update_flags),
                 iewa,
                 insert_overwrite,
                 item_ts_for_expire)) {
  ...
  db_kernel->Update(
      ...
      sizeof(items),
      0,
      [&](uint32 old_addr, uint64 key, const char *log, int log_size, int dummy_t, base::MemKV *mem_kv)
          -> uint32 { return updater->UpdateHandler(old_addr, key, log, log_size, 0, mem_kv); },
      nullptr,
      [&](uint64 cur_key, base::KVData *data) -> void { updater->RecycleHandler(cur_key, data); });
...
ErrorCode GraphKV::DeleteRelations(uint64 in_id,
  ...
  for (size_t i = 0; i < out_ids.size(); ++i) {
    if (out_ids[i] == 0) {
      LOG_EVERY_N_SEC(WARNING, 1) << "Delete an invalid edge key, ignore it. "
  ...
  db_kernel->Update(
      ...
      sizeof(items),
      0,
      [&](uint32 addr, uint64 key, const char *log, int log_size, int dummy_t, base::MemKV *mem_kv)
          -> uint32 { return updater->DeleteEdgeHandler(addr, key, log, log_size, 0, mem_kv); },
      nullptr,
      [&](uint64 cur_key, base::KVData *data) -> void { updater->RecycleHandler(cur_key, data); });
  return ErrorCode::OK;
}

ErrorCode GraphKV::RandomSample(std::string relation_name,
                                int node_count,
  ...
  if (!pool_name.empty()) {
    if (custom_rand_samplers_.find(pool_name) != custom_rand_samplers_.end()) {
      ...
      status =
          custom_rand_samplers_.at(pool_name)->Sample(relation_name, node_count, node_ids->mutable_data());
      used_custom_pool = true;
    } else {
      LOG_EVERY_N_SEC(WARNING, 10) << "custom pool " << pool_name
  ...
  if (!used_custom_pool) {
    if (global_rand_sampler_.get()) {
      ...
      node_ids->Resize(node_count, 0);
      status = global_rand_sampler_->Sample(relation_name, node_count, node_ids->mutable_data());
    }
  }
...
// Even if sampling failed, the client usually will padding the result to make sure the total
// number of result is equal to the sampling num.
ErrorCode GraphKV::SampleNeighbors(uint64 id,
                                   std::string relation_name,
  ...
  PERF_SUM(1,
           relation_name,
           P_RQ,
           "sample.type-" + SamplingParam_SamplingStrategy_Name(sampling_param.strategy()),
           caller_service_name);
  if (value.data == nullptr || value.expired()) {
    ...
    LOG_EVERY_N_SEC(INFO, 2) << "sample node not exist or expired, id = " << id
                             << ", relation_name = " << relation_name << ", expired = " << value.expired();
  ...
  if (value.size == 0) {
    ...
    LOG_EVERY_N_SEC(INFO, 2) << "sample node has empty node list, stop early, id = " << id
                             << ", relation_name = " << relation_name;
    return ErrorCode::OK;
  }
  ...
  auto updater = updaters_.at(relation_name).get();
  bool ret = updater->edge_list_adaptor_->Sample(value.data, sampling_param, result_writer);
  timer.AppendCostMs("sample one id");
  monitor_manager_->AddSampleResultCount(result_writer->offset - prev, relation_name, caller_service_name);
  ...
  auto eps = timer.Stop();
  if (eps > 2000) {
    LOG_EVERY_N_SEC(WARNING, 3) << "sample key: " << id << " long time cost: " << timer.display()
                                << ", sample type: " << sampling_param.strategy();

  } else {
    LOG_EVERY_N_SEC(INFO, 5) << "sample key: " << id << " time cost: " << timer.display()
                             << ", sample type: " << sampling_param.strategy();
  }
  return ErrorCode::OK;
}
...
ErrorCode GraphKV::GetNodeInfos(const ProtoList<uint64> &src_ids,
  ...
  LOG_EVERY_N_SEC(INFO, 5) << "use_ms: " << (end - begin) << ", src_id num: " << src_ids.size();
  return ErrorCode::OK;
}
...
ErrorCode GraphKV::QueryNeighbors(uint64 src_id,
  ...
  bool key_not_exist = value.data == nullptr || value.size == 0 || value.expired();
  // Get all edges.
  if (dst_ids.empty()) {
    if (key_not_exist) { return ErrorCode::KEY_NOT_EXIST; }
    node_attr->set_id(src_id);
    bool succ = updater->edge_list_adaptor_->GetEdgeListInfo(value.data, node_attr, edge_info);
    if (!succ) { return ErrorCode::UNKNOWN; }
    return ErrorCode::OK;
  }
  ...
  bool succ = updater->edge_list_adaptor_->GetEdges(value.data, dst_ids, node_attr, edge_info);
  if (!succ) { return ErrorCode::UNKNOWN; }
  return ErrorCode::OK;
}

ErrorCode GraphKV::TraversalByPart(std::string relation_name,
                                   int64 part_num,
                                   int64 part_id,
                                   ProtoList<uint64> *result) {
  SAFE_DB_BY_RELATION_RETURN_CODE()
  std::vector<std::vector<uint64>> shard_keys(db_kernel->PartNum(), std::vector<uint64>{});
  size_t key_size = 0;
  ...
  for (auto &keys : shard_keys) {
    for (auto key : keys) { result->AddAlreadyReserved(key); }
    offset += keys.size();
  }
...
ErrorCode GraphKV::GetRelationNodeCount(std::string relation_name, int64 *count) {
  SAFE_DB_BY_RELATION_RETURN_CODE()
...
uint64 GraphKV::GetNodeDegree(chrono_graph::GraphNode info) {
  ...
  if (value.size <= 0 || value.expired()) { return 0; }
...
std::string GraphKV::GetStatInfo() const {
  ...
  for (const auto &pair : dbs_) {
    ...
    if (!kv) { continue; }
    buffer << std::endl
           << ">>>> \nRelation = " << relation_name << ", keys num = " << kv->ValidKeyNum()
...
void GraphKV::ReduceDbMemory(const GraphStorageDBConfig *config,
  ...
  int64_t key_num_threshold = key_num * (1 - config->reduce_occupancy_rate / occupancy_rate);
  float bucket_interval = 1.0, threshold_score = 0, left_threshold = 0, right_threshold = 1e9;
  ...
  if (config->reduce_occupancy_strategy == 1) {
    ...
    right_threshold = 5e9;
    // If kScoreBucketNum = 32768, and every 1 minute's data will be put into the same bucket,
    // then we can cover about 22.75 days' data. If all data will not expire in the future 22.75 days, the
    // expiration mechanism will fail.
  ...
  while (sum < key_num_threshold) {
    for (size_t i = 0; i < kScoreBucketNum; ++i) { buckets[i] = 0; }
    for (int part_i = 0; part_i < mem_kv->PartNum(); ++part_i) {
      mem_kv->EraseIf(part_i, [&](uint64 key, base::KVData *data) {
        ...
        if (score >= left_threshold && score < right_threshold) {
          ...
          int bucket_number = std::min(bucket_score, static_cast<float>(kScoreBucketNum - 1));
    ...
    while (i > 0 && sum + buckets[i] < key_num_threshold) { sum += buckets[i--]; }
  ...
  uint64 max_erase_num =
      mem_kv->ValidKeyNum() * (config->reduce_occupancy_rate / mem_kv->ValidOccupancyRate());
  std::atomic<uint64_t> erase_sum(0);
  int total_erase_num = 0;
  for (int part_i = 0; part_i < mem_kv->PartNum(); ++part_i) {
    erase_sum = 0;
    total_erase_num +=
        mem_kv->EraseIf(part_i, [&, max_erase_num, threshold_score](uint64 key, base::KVData *data) {
          float score = calc_score_func(data);
          return score < threshold_score && erase_sum++ < max_erase_num / mem_kv->PartNum();
        });
  }
  ...
  LOG(INFO) << config->relation_name << " Before occupancy_rate: " << occupancy_rate
            << ", key_num: " << key_num << ", threshold_score: " << std::setprecision(12) << threshold_score
            << ", erase_num: " << total_erase_num << ". use_ms: " << use_ms << ". loop_time: " << loop_time;
}
...
void GraphKV::DetectionDb(std::string relation) {
  while (!stop_) {
    ...
    LOG_EVERY_N_SEC(INFO, 600) << "Relation " << relation << " info: " << db->GetInfo();
    ReduceDbMemory(db_configs_[relation], db, updater);
    for (size_t i = 0; i < absl::GetFlag(FLAGS_detect_db_interval_in_seconds); ++i) {
      if (stop_) { break; }
      base::SleepForSeconds(1);
    }
  }
...
void GraphKV::TimeDecayThread(std::string relation) {
  auto db_kernel = GetKVByType(dbs_, relation);
  CHECK(db_kernel);
  while (!stop_) {
    auto mem_kv = dbs_[relation].get();
    if (!mem_kv || !db_configs_[relation]->need_time_decay()) {
      LOG_EVERY_N_SEC(INFO, 300) << "TimeDecayThread for " << relation << " has nothing to do.";
    ...
    auto updater = updaters_[relation].get();
    uint64 interval = db_configs_[relation]->decay_interval_s * 1000000ul;
    uint64 next_ts = base::GetTimestamp() / interval * interval + interval;
    while (base::GetTimestamp() < next_ts) {
      if (stop_) return;
      LOG_EVERY_N_SEC(INFO, 60) << "TimeDecayThread for " << relation << " waiting, remaining seconds: "
    ...
    for (int part_i = 0; part_i < mem_kv->PartNum(); ++part_i) {
      mem_kv->TraverseOp(part_i, [&](uint64 key, base::KVData *data) {
        // Use Update to keep memkv's modify lock
        mem_kv->Update(
            key,
            nullptr,
            0,
            ...
            [&](uint32 old_addr,
              ...
              return updater->TimeDecayHandler(old_addr,
                                               key,
                                               &delete_count,
                                               db_configs_[relation]->degree_decay_ratio,
                                               db_configs_[relation]->weight_decay_ratio,
                                               db_configs_[relation]->delete_threshold_weight,
                                               mem_kv);
            },
            nullptr,
            [&](uint64 cur_key, base::KVData *data) -> void { updater->RecycleHandler(cur_key, data); });
    ...
    LOG(INFO) << "TimeDecayThread for " << relation << " delete edge num: " << delete_count;
    PERF_SUM(delete_count, relation, "time_decay.delete_count");
  }
}

// Similar to TimeDecayThread
void GraphKV::EdgeExpireThread(std::string relation) {
  auto db_kernel = GetKVByType(dbs_, relation);
  CHECK(db_kernel);
  while (!stop_) {
    auto mem_kv = dbs_[relation].get();
    if (!mem_kv || !db_configs_[relation]->need_edge_expire()) {
      LOG_EVERY_N_SEC(INFO, 300) << "EdgeExpireThread for " << relation << " has nothing to do.";
      for (size_t i = 0; i < 5; ++i) {
        if (stop_) { break; }
        base::SleepForSeconds(1);
      }
    ...
    auto updater = updaters_[relation].get();
    uint64 interval = db_configs_[relation]->edge_expire_check_interval_s * 1000000ul;
    uint64 next_ts = base::GetTimestamp() / interval * interval + interval;
    while (base::GetTimestamp() < next_ts) {
      if (stop_) return;
      LOG_EVERY_N_SEC(INFO, 60) << "EdgeExpireThread for " << relation << " waiting, remaining seconds: "
                                << ConvertTimestampToSec(next_ts) -
                                       ConvertTimestampToSec(base::GetTimestamp());
      base::SleepForSeconds(1);
    }
        ...
        mem_kv->Update(
            ...
            [&](uint32 old_addr,
              ...
              return updater->EdgeExpireHandler(
                  old_addr, key, &delete_count, db_configs_[relation]->edge_expire_interval_s, mem_kv);
            },
            nullptr,
            [&](uint64 cur_key, base::KVData *data) -> void { updater->RecycleHandler(cur_key, data); });
        return false;
    ...
    LOG(INFO) << "EdgeExpireThread for " << relation << " delete edge num: " << delete_count;
    PERF_SUM(delete_count, relation, "edge_expire.delete_count");
  }
}
...
static base::MultiMemKV *GetKVByType(
    const std::unordered_map<std::string, std::unique_ptr<base::MultiMemKV>> &db, std::string relation_name) {
  if (db.find(relation_name) != db.end()) { return db.at(relation_name).get(); }
  return nullptr;
}
