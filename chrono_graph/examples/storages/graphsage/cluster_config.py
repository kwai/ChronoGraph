...
experiment_name = "graphsage_demo"
shm_size = (1 << 30) * 32
...
relations = {
    "U2I": {
        ...
        "total_memory": (1 << 30) * 16,
        "key_size": 1000000,
        # max edges for each node
        "edge_max_num": 500,
        # when length of edge list exceeds max num
        # 0 = pop oldest, 1 = pop random, 2 = pop smallest weight, 3 = cancel insert
        "oversize_replace_strategy": 2,
        # node expire time, 0 is never expire
        "expire_interval": 0,
        "elst": "cpt",
    },
    "I2U": {
...
def main():
  ...
  total_part_shard_num = sum(
      [item["shard_num"] if "shard_num" in item else 1 for item in part_configs])
