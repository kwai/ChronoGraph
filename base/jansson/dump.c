...
static int dump_to_file(const char *buffer, int size, void *data) {
  FILE *dest = (FILE *)data;
  if (fwrite(buffer, size, 1, dest) != 1) return -1;
  return 0;
}
...
static int dump_string(const char *str, dump_func dump, void *data) {
  ...
  while (1) {
    ...
    while (*end && *end != '\\' && *end != '"' && (*end < 0 || *end > 0x1F)) end++;

    if (end != str) {
      if (dump(str, end - str, data)) return -1;
    }
    ...
    if (dump(text, length, data)) return -1;
...
static int do_dump(const json_t *json, uint32_t flags, int depth, dump_func dump, void *data) {
  switch (json_typeof(json)) {
    ...
    case JSON_INTEGER: {
      ...
      size = asprintf(&buffer, "%ld", json_integer_value(json));
      if (size == -1) return -1;
    ...
    case JSON_ARRAY: {
      ...
      if (dump("[", 1, data)) return -1;
      if (n == 0) return dump("]", 1, data);
      if (dump_indent(flags, depth + 1, dump, data)) return -1;

      for (i = 0; i < n; ++i) {
        if (do_dump(json_array_get(json, i), flags, depth + 1, dump, data)) return -1;

        if (i < n - 1) {
          if (dump(",", 1, data) || dump_indent(flags, depth + 1, dump, data)) return -1;
        } else {
          if (dump_indent(flags, depth, dump, data)) return -1;
        }
      }
      return dump("]", 1, data);
    }
...
char *json_dumps(const json_t *json, uint32_t flags) {
  ...
  if (!json_is_array(json) && !json_is_object(json)) return NULL;

  if (strbuffer_init(&strbuff)) return NULL;

  if (do_dump(json, flags, 0, dump_to_strbuffer, (void *)&strbuff)) return NULL;

  if (dump_to_strbuffer("\n", 1, (void *)&strbuff)) return NULL;
...
int json_dumpf(const json_t *json, FILE *output, uint32_t flags) {
  if (!json_is_array(json) && !json_is_object(json)) return -1;

  if (do_dump(json, flags, 0, dump_to_file, (void *)output)) return -1;
  return dump_to_file("\n", 1, (void *)output);
}
