...
#include "base/jansson/jansson_private.h"
#include "base/jansson/strbuffer.h"
#include "base/jansson/utf.h"
...
static char stream_get(stream_t *stream, json_error_t *error) {
  ...
  if (!stream->buffer[stream->buffer_pos]) {
    ...
    if (c == EOF && stream->eof(stream->data)) return EOF;
    ...
    if (c < 0) {
      ...
      if (!utf8_check_full(stream->buffer, count)) goto out;
...
static int lex_eof(lex_t *lex) { return lex->stream.eof(lex->stream.data); }

static void lex_save(lex_t *lex, char c) { strbuffer_append_byte(&lex->saved_text, c); }
...
static void lex_scan_string(lex_t *lex, json_error_t *error) {
  ...
  while (c != '"') {
    if (c == EOF) {
      if (lex_eof(lex)) error_set(error, lex, "premature end of input");
      goto out;
    }
  ...
  while (*p != '"') {
    if (*p == '\\') {
      ...
      if (*p == 'u') {
        ...
        if (utf8_encode(value, buffer, &length)) assert(0);
...
static void lex_scan_number(lex_t *lex, char c, json_error_t *error) {
  ...
  if (c == '0') {
    ...
    c = lex_get_save(lex, error);
    while (isdigit(c)) c = lex_get_save(lex, error);
  }
...
static int lex_scan(lex_t *lex, json_error_t *error) {
  ...
  if (c == '{' || c == '}' || c == '[' || c == ']' || c == ':' || c == ',')
    ...
    c = lex_get_save(lex, error);
    while (isupper(c) || islower(c)) c = lex_get_save(lex, error);
    lex_unget_unsave(lex, c);
...
static void lex_close(lex_t *lex) {
  if (lex->token == TOKEN_STRING) free(lex->value.string);
...
json_t *json_loads(const char *string, json_error_t *error) {
  ...
  string_data_t stream_data = {.data = string, .pos = 0};

  if (lex_init(&lex, string_get, string_eof, (void *)&stream_data)) return NULL;
...
json_t *json_loadf(FILE *input, json_error_t *error) {
  ...
  if (lex_init(&lex, (get_func)fgetc, (eof_func)feof, input)) return NULL;
...
static int buffer_eof(void *data) {
  buffer_data_t *stream = (buffer_data_t *)data;
  return (stream->pos >= stream->len);
}

json_t *json_loadb(const char *buffer, size_t buflen, json_error_t *error) {
  lex_t lex;
  json_t *result;
  ...
  if (lex_init(&lex, buffer_get, buffer_eof, (void *)&stream_data)) return NULL;
