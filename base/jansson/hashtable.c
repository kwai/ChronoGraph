...
static unsigned int primes[] = {
    5,       13,       23,       53,       97,        193,       389,       769,       1543,      3079,
...
static inline unsigned int num_buckets(hashtable_t *hashtable) { return primes[hashtable->num_buckets]; }

static pair_t *hashtable_find_pair(hashtable_t *hashtable,
                                   bucket_t *bucket,
                                   const void *key,
                                   unsigned int hash) {
  list_t *list;
  ...
  if (bucket_is_empty(hashtable, bucket)) return NULL;

  list = bucket->first;
  while (1) {
    pair = list_to_pair(list);
    if (pair->hash == hash && hashtable->cmp_keys(pair->key, key)) return pair;
...
static int hashtable_do_del(hashtable_t *hashtable, const void *key, unsigned int hash) {
  pair_t *pair;
  bucket_t *bucket;
  unsigned int index;
  ...
  if (hashtable->free_key) hashtable->free_key(pair->key);
  if (hashtable->free_value) hashtable->free_value(pair->value);
...
int hashtable_init(
    hashtable_t *hashtable, key_hash_fn hash_key, key_cmp_fn cmp_keys, free_fn free_key, free_fn free_value) {
  unsigned int i;
...
int hashtable_set(hashtable_t *hashtable, void *key, void *value) {
  ...
  if (hashtable->size >= num_buckets(hashtable))
    if (hashtable_do_rehash(hashtable)) return -1;
...
void *hashtable_iter(hashtable_t *hashtable) { return hashtable_iter_next(hashtable, &hashtable->list); }

void *hashtable_iter_next(hashtable_t *hashtable, void *iter) {
  list_t *list = (list_t *)iter;
  if (list->next == &hashtable->list) return NULL;
  return list->next;
}
