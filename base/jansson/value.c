...
static void value_decref(void *value) { json_decref((json_t *)value); }
...
json_t *json_object(void) {
  ...
  if (hashtable_init(&object->hashtable, hash_string, string_equal, free, value_decref)) {
    free(object);
    return NULL;
  }
  return &object->json;
...
int json_object_set(json_t *json, const char *key, json_t *value) {
  if (!utf8_check_string(key, -1)) return -1;
...
json_t *json_string(const char *value) {
  if (!utf8_check_string(value, -1)) return NULL;
...
static void json_delete_integer(json_integer_t *integer) { free(integer); }
...
static void json_delete_real(json_real_t *real) { free(real); }
...
json_t *json_true(void) {
  static json_t the_true = {.type = JSON_TRUE, .refcount = 1};
  return json_incref(&the_true);
}

json_t *json_false(void) {
  static json_t the_false = {.type = JSON_FALSE, .refcount = 1};
  return json_incref(&the_false);
}

json_t *json_null(void) {
  static json_t the_null = {.type = JSON_NULL, .refcount = 1};
